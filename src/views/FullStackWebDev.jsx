  ...
  >
    View Portfolio→
      ...
      <section className="fswd__stats_section">
        <div className="fswd__container"><div className="fswd__stats_border_box">{stats.map((s, i) => (<div key={i} className="stat_item"><span className="stat_num">{s.number}</span><span className="stat_lbl">{s.label}</span></div>))}</div></div>
      </section>
      <section className="fswd__section white_bar">
        <div className="fswd__container">
          <h2 className="fswd__title_dark">What Is Full Stack Web Development?</h2>
          <div className="fswd__intro_content"><p>Full stack web development covers both sides of a web application — the frontend that users see and interact with, and the backend that powers the logic, data and integrations behind the scenes.</p><p>For businesses in the UK and USA, working with a full stack development agency means one team handles everything — from database architecture and API development to user interface design and cloud deployment.</p></div>
          <div className="fswd__grid_3">{whatIsItems.map((item, i) => (<div key={i} className="fswd__service_box hover_lift"><div className="fswd__box_icon_circle">{item.i}</div><h4 className="fswd__box_title">{item.title}</h4><p className="fswd__justify_text">{item.description}</p></div>))}</div>
        </div>
      </section>
      <section className="fswd__section grey_bar">
        <div className="fswd__container">
          <h2 className="fswd__title_dark">Our Full Stack Web Development Services</h2>
          <div className="fswd__grid_3">{services.map((s, i) => (<div key={i} className="fswd__service_box hover_lift"><div className="fswd__box_icon">{s.i}</div><h4 className="fswd__box_title">{s.t}</h4><ul className="fswd__card_bullets">{s.points.map((p, idx) => <li key={idx}><span>›</span> {p}</li>)}</ul></div>))}</div>
        </div>
      </section>
      <section className="fswd__section white_bar">
        <div className="fswd__container">
          <h2 className="fswd__title_dark">Technologies We Use</h2>
          <div className="fswd__grid_4">{techStack.map((t, i) => (<div key={i} className="tech_box_full hover_lift"><div className="tech_box_head"><img src={t.icon} alt={t.c} style={{ width: '32px', height: '32px', objectFit: 'contain' }} />{t.c}</div><div className="tech_pills_wrap">{t.items.map((item, j) => <span key={j} className="tech_pill">{item}</span>)}</div></div>))}</div>
        </div>
      </section>
      <section className="fswd__section grey_bar">
        <div className="fswd__container">
          <h2 className="fswd__title_dark">Why UK & USA Businesses Hire ZonzocTech</h2>
          <div className="fswd__grid_3">{whyChoose.map((w, i) => (<div key={i} className="fswd__service_box hover_lift"><div className="fswd__box_icon">{w.i}</div><h4 className="fswd__box_title">{w.t}</h4><ul className="fswd__card_bullets">{w.points.map((p, idx) => <li key={idx}><span>›</span> {p}</li>)}</ul></div>))}</div>
        </div>
      </section>
      <section className="fswd__section white_bar">
        <div className="fswd__container">
          <h2 className="fswd__title_dark">Development Process</h2>
          <div className="fswd__roadmap_grid">{processSteps.map((step, i) => (<div key={i} className="fswd__roadmap_card"><div className="roadmap_node"><div className="roadmap_circle">{step.n}</div></div><div className="roadmap_content_box hover_lift"><h4>{step.t}</h4><p className="fswd__justify_text">{step.d}</p></div></div>))}</div>
        </div>
      </section>
      <section className="fswd__section grey_bar">
        <div className="fswd__container">
          <h2 className="fswd__title_dark">Frequently Asked Questions</h2>
          <div className="fswd__faq_narrow">{faqs.map((f, i) => (<details key={i} className="faq_item"><summary><div><span>{i + 1}.</span> {f.q}</div> <span className="faq_plus">+</span></summary><div className="faq_ans fswd__justify_text">{f.a}</div></details>))}</div>
        </div>
     </section>
{/* MID CTA SECTION */}
